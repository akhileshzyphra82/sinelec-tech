    ...
    <div class="account-shell">
      ...
      <section class="account-main da-main">
            ...
            <div class="da-grid">
              ...
              <div class="da-field">
                ...
                <select id="accountAddrCountry" class="da-input" autocomplete="country-name">
                  ...
                  <option value="<?= htmlspecialchars((string)($c->COUNTRY ?? '')) ?>" data-cid="<?= (int)($c->COUNTRY_ID ?? 0) ?>"><?= htmlspecialchars((string)($c->COUNTRY ?? '')) ?></option>
                  <?php endforeach; ?>
                </select>
              ...
              <div class="da-field da-field--full">
                <div class="da-row3">
                  <div class="da-field">
                    ...
                    <input type="text" id="accountAddrPin" class="da-input" placeholder="Postal code" maxlength="10" autocomplete="postal-code">
              ...
              <div class="da-field da-field--full">
                <label for="accountAddrExtra">Additional Delivery Notes</label>
                <textarea id="accountAddrExtra" class="da-input da-textarea" placeholder="Gate code, special instructions, access notes…"></textarea>
              </div>
        ...
        <div class="da-list" id="accountAddressGrid">
          <div class="da-loading">Loading addresses…</div>
        </div>
...
.da-form-card {
  ...
  border-radius: 14px; box-shadow: 0 2px 12px rgba(15,30,55,.06);
...
.da-form-card-head {
  ...
  font-size: 12px; font-weight: 700; color: #3a4f66;
  letter-spacing: .04em; text-transform: uppercase;
}
.da-form-card form { padding: 22px; }

/* ─── Address label tabs ──────────────────────────────────── */
.da-tabs-group { margin-bottom: 22px; }
.da-tabs-label { font-size: 11px; font-weight: 600; color: #5b6b7c; margin-bottom: 8px; }
.da-tabs {
  display: inline-flex; gap: 0;
  background: #f0f4f8; border-radius: 11px; padding: 4px;
}
.da-tab {
  display: inline-flex; align-items: center; gap: 6px;
  height: 36px; padding: 0 18px;
  border-radius: 8px; font-size: 12px; font-weight: 600;
  color: #5b6b7c; background: transparent;
...
.da-field label { font-size: 11px; font-weight: 600; color: #5b6b7c; line-height: 1; }
.da-ast { color: #e53935; }
.da-input {
...
.da-input:focus { border-color: #2563eb; box-shadow: 0 0 0 3px rgba(37,99,235,.12); }
...
.da-hint { font-size: 10.5px; min-height: 14px; padding-left: 1px; line-height: 1.3; }
...
.da-section-sep {
  ...
  padding: 6px 0 2px; border-top: 1px solid #edf0f5; margin-top: 6px;
}
.da-section-name { font-size: 11.5px; font-weight: 700; color: #2d4160; letter-spacing: .02em; }
.da-optional-pill {
  font-size: 10px; font-weight: 600; color: #7a92ab;
  background: #eef2f7; padding: 2px 9px; border-radius: 20px;
}
.da-row3 {
...
.da-btn--primary { background: #2563eb; color: #fff; border: 1.5px solid #2563eb; }
.da-btn--primary:hover { background: #1d4ed8; border-color: #1d4ed8; }
.da-btn--primary:disabled { background: #93b4f3; border-color: #93b4f3; cursor: not-allowed; }
.da-btn--ghost { background: #fff; color: #3a4f66; border: 1.5px solid #d0d7e2; }
.da-btn--ghost:hover { border-color: #a0aec0; color: #1a2332; }

/* ─── Loading ─────────────────────────────────────────────── */
.da-loading {
  padding: 36px 20px; text-align: center;
  font-size: 13px; color: #7a92ab;
}
...
.da-card {
  background: #fff; border: 1px solid #e2e8f0;
  border-radius: 12px; box-shadow: 0 1px 6px rgba(15,30,55,.05);
...
.da-card-head {
  ...
  background: #f8fafc; border-bottom: 1px solid #edf0f5;
  border-radius: 12px 12px 0 0;
}
.da-badge-row { display: flex; align-items: center; gap: 7px; flex-wrap: wrap; }
.da-badge {
...
.da-badge--other  { background: #fef9c3; color: #854d0e; }

/* 3-dot action menu */
.da-menu { position: relative; }
.da-menu-trigger {
  display: flex; align-items: center; justify-content: center;
  width: 32px; height: 32px; border-radius: 7px;
  color: #7a92ab; font-size: 18px; line-height: 1;
...
.da-menu-item {
  ...
  font-size: 12px; font-weight: 600; color: #2d4160;
...
.da-card-company { font-size: 15px; font-weight: 700; color: #1a2332; margin-bottom: 4px; }
.da-card-meta {
  display: flex; flex-wrap: wrap; gap: 4px 18px;
  font-size: 12px; color: #4a6179; margin-bottom: 10px;
}
.da-card-meta span { display: inline-flex; align-items: center; gap: 4px; }
.da-card-address { font-size: 13px; color: #3a5168; line-height: 1.55; margin-top: 6px; }
.da-card-extra {
  margin-top: 8px; padding: 7px 10px;
  background: #fffbeb; border-left: 3px solid #f59e0b;
  border-radius: 0 6px 6px 0;
  font-size: 11.5px; color: #78501a; font-style: italic;
}

/* Recipient footer */
.da-card-recipient {
  margin: 12px -18px -16px; padding: 10px 18px;
  background: #f8fafc; border-top: 1px solid #edf0f5;
  display: flex; flex-wrap: wrap; align-items: center; gap: 4px 18px;
}
.da-recipient-label {
  font-size: 10px; font-weight: 700; color: #7a92ab;
  text-transform: uppercase; letter-spacing: .04em;
  flex-basis: 100%; margin-bottom: 2px;
}
.da-recipient-item {
  font-size: 12px; color: #3a5168;
...
.da-empty {
  ...
  background: #fff; border: 1px dashed #d0d7e2;
...
/* ─── Responsive ──────────────────────────────────────────── */
@media (max-width: 860px) { .da-row3 { grid-template-columns: 1fr 1fr; } }
...
@media (max-width: 520px) {
  ...
  .da-card-company { font-size: 14px; }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var countrySelEl = document.getElementById('accountAddrCountry');
  ...
  /* ── Country select → hidden country_id ── */
  if (countrySelEl) {
    countrySelEl.addEventListener('change', function () {
      var opt = this.options[this.selectedIndex];
      countryIdInp.value = opt ? (opt.dataset.cid || '0') : '0';
    });
  }
  function setCountry(countryName, countryId) {
    if (!countrySelEl) return;
    var opts = countrySelEl.options;
    for (var i = 0; i < opts.length; i++) {
      if (opts[i].value === countryName || (countryId && opts[i].dataset.cid == countryId)) {
        countrySelEl.selectedIndex = i;
        countryIdInp.value = opts[i].dataset.cid || '0';
        return;
      }
    }
    countrySelEl.selectedIndex = 0;
    countryIdInp.value = '0';
  }
  ...
  function clearEdit() {
    addrIdInp.value = '';
    submitBtn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M5 12l5 5L19 7" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg> Save Address';
    cancelBtn.hidden = true;
    setHint('', '');
    setActiveTab('Home');
    countrySelEl && (countrySelEl.selectedIndex = 0);
    countryIdInp && (countryIdInp.value = '0');
  }
  ...
  function renderCard(a) {
    var hasRecipient = a.recipient_name || a.recipient_email || a.recipient_contact;
    var addrParts = [];
    if (a.address_line_one) addrParts.push(a.address_line_one);
    if (a.address_line_two) addrParts.push(a.address_line_two);
    if (a.landmark) addrParts.push('Near: ' + a.landmark);
    var cityLine = [a.city, a.state ? a.state + (a.zip ? ' - ' + a.zip : '') : a.zip].filter(Boolean).join(', ');
    ...
    return '<article class="da-card">'
      ...
      +     '<span class="da-badge ' + badgeClass(a.label) + '">' + badgeIcon(a.label) + esc(a.label || 'Other') + '</span>'
      +   '</div>'
      +   '<div class="da-menu" style="position:relative">'
      +     '<details>'
      +       '<summary class="da-menu-trigger" aria-label="Actions">&#8942;</summary>'
      +       '<div class="da-menu-pop">'
      +         '<button type="button" class="da-menu-item" data-addr-edit="' + esc(a.id) + '">'
      +           '<svg width="12" height="12" viewBox="0 0 24 24" fill="none"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Edit'
      +         '</button>'
      +         '<button type="button" class="da-menu-item da-menu-item--danger" data-addr-delete="' + esc(a.id) + '">'
      +           '<svg width="12" height="12" viewBox="0 0 24 24" fill="none"><polyline points="3 6 5 6 21 6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><path d="M10 11v6M14 11v6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> Delete'
      +         '</button>'
      +       '</div>'
      +     '</details>'
      +   '</div>'
      ...
      +   '<div class="da-card-company">' + esc(a.company_name || a.user_name || '—') + '</div>'
      +   '<div class="da-card-meta">'
      +     (a.user_name
              ? '<span><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="12" cy="7" r="4" stroke="currentColor" stroke-width="2"/></svg> ' + esc(a.user_name) + '</span>' : '')
      +     (a.delivery_phone_no
              ? '<span><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> ' + esc((a.mobile_country_code > 0 ? '+' + a.mobile_country_code + ' ' : '') + a.delivery_phone_no) + '</span>' : '')
      +   '</div>'
      +   '<p class="da-card-address">' + esc(addrParts.join(', ')) + '</p>'
      +   (a.address ? '<div class="da-card-extra">' + esc(a.address) + '</div>' : '')
      +   (hasRecipient
            ? '<div class="da-card-recipient">'
              + '<span class="da-recipient-label">Recipient</span>'
              + (a.recipient_name    ? '<span class="da-recipient-item"><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><circle cx="12" cy="7" r="4" stroke="currentColor" stroke-width="2"/></svg> ' + esc(a.recipient_name)    + '</span>' : '')
              + (a.recipient_contact ? '<span class="da-recipient-item"><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> ' + esc(a.recipient_contact) + '</span>' : '')
              + (a.recipient_email   ? '<span class="da-recipient-item"><svg width="11" height="11" viewBox="0 0 24 24" fill="none"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><polyline points="22 6 12 13 2 6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg> ' + esc(a.recipient_email)   + '</span>' : '')
              + '</div>'
            : '')
      + '</div>'
      + '</article>';
      ...
      .then(function (r) {
        return r.text().then(function (txt) {
          try { return JSON.parse(txt); }
            ...
            return { ok: false, msg: 'Server error (invalid response). Check console.' };
          }
        });
      })
      .then(cb)
  ...
  function renderList(list) {
    ...
    grid.innerHTML = addrList.map(renderCard).join('');
  }

  function loadList() {
    grid.innerHTML = '<div class="da-loading">Loading addresses…</div>';
    ajaxPost({ action: 'get_list' }, function (res) {
      if (res.ok) {
        renderList(res.list);
      } else {
        grid.innerHTML = '<div class="da-empty"><p>Could not load addresses: ' + (res.msg || 'unknown error') + '</p></div>';
      }
    });
  }
  ...
  form.addEventListener('submit', function (e) {
    ...
    if (!country || !fld('accountAddrFullName') || !line1 || !zip || !city) {
      showErr('Please fill all required fields marked with *.');
      return;
    }
    ...
    if (editId) {
      ...
      payload.user_address_id = editId;
      ajaxPost(payload, function (res) {
        submitBtn.disabled = false;
        if (!res.ok) { showErr(res.msg || 'Update failed.'); return; }
        form.reset(); clearEdit(); closePanel(); renderList(res.list);
  ...
  resetBtn && resetBtn.addEventListener('click', function () {
    setTimeout(function () { clearEdit(); showErr(''); }, 0);
  });
  cancelBtn && cancelBtn.addEventListener('click', function () {
    form.reset(); clearEdit(); closePanel(); showErr('');
  });
  ...
  grid.addEventListener('click', function (e) {
    ...
    if (editEl) {
      var id  = parseInt(editEl.dataset.addrEdit, 10);
      var obj = addrList.find(function (a) { return a.id === id; }) || null;
      if (obj) { enterEdit(obj); }
      return;
    }

    if (deleteEl) {
      var id = parseInt(deleteEl.dataset.addrDelete, 10);
